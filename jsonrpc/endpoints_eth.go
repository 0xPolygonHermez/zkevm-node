...
func NewEthEndpoints(cfg Config, chainID uint64, p types.PoolInterface, s types.StateInterface, etherman types.EthermanInterface) *EthEndpoints {
	e := &EthEndpoints{cfg: cfg, chainID: chainID, pool: p, state: s, etherman: etherman, storage: NewStorage()}
...
func (e *EthEndpoints) notifyNewHeads(event state.NewL2BlockEvent) {
	...
	e.storage.SendBlockFilterSubscriptionData(data)
...
func (e *EthEndpoints) notifyNewLogs(event state.NewL2BlockEvent) {
	...
	for _, filter := range e.storage.LogFilters {
		...
		if filterParameters.FromBlock == nil && filterParameters.ToBlock == nil {
			...
			if filterParameters.FromBlock != nil {
				...
				if rpcErr != nil {
					log.Errorf(rpcErr.Error(), filter.ID)
					continue
				}
