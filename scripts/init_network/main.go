...
func sendL2Claim(ctx context.Context, auth *bind.TransactOpts, client *ethclient.Client, deposit Deposit, smtProof [][32]byte, globalExitRoot *globalExitRoot) {
	...
	tx, err := br.Claim(auth, deposit.TokenAddr, deposit.Amount, deposit.OrigNet, deposit.DestNet,
		deposit.DestAddr, smtProof, deposit.DepositCnt, globalExitRoot.GlobalExitRootNum,
