...
func (g *LastNBatches) GetAvgGasPrice(ctx context.Context) (*big.Int, error) {
	batchNumber, err := g.state.GetLastL2BlockNumber(ctx, nil)
	if err != nil {
...
func (g *LastNBatches) getBatchTxsTips(ctx context.Context, blockNum uint64, limit int, ignorePrice *big.Int, result chan results, quit chan struct{}) {
	txs, err := g.state.GetTxsByL2BlockNumber(ctx, blockNum, nil)
	if txs == nil {
		select {
