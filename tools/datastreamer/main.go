...
func rebuild(cliCtx *cli.Context) error {
	...
	tx, _, err := state.DecodeTx(string(txEntry.Data[6:]), forkID)
	if err != nil {
		log.Fatal(err)
	}
...
func printEntry(entry datastreamer.FileEntry, streamServer *datastreamer.StreamServer) {
	...
	case state.EntryTypeL2Tx:
		...
		previousEntry, err := streamServer.GetEntry(entry.Number - 1)
		if err != nil {
			log.Fatal(err)
		}
		forkID := uint64(binary.LittleEndian.Uint16(previousEntry.Data[76:78]))

		tx, _, err := state.DecodeTx(common.Bytes2Hex(encode), forkID)
		if err != nil {
			log.Fatal(err)
		}
