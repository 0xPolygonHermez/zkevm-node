...
Block Scout requires access to the `Hermez zk-EVM-Node` instance to have access to the network
...
Create a file called `docker-compose.yml` inside of the `hermez` directory.
...
> Remember to set the environment variable ETHEREUM_JSONRPC_HTTP_URL with the `Hermez zk-EVM-Node` IP and PORT
...
services:
    ...
    hez-explorer:
        ...
        environment:
            ...
            - ETHEREUM_JSONRPC_HTTP_URL=http://:8545 # Set the IP and PORT of the Hermez zk-EVM-Node
...
```bash
docker-compose up -d hez-explorer-postgres
sleep5
docker-compose up -d hez-explorer
```
