...
func TestExecuteWithoutUpdatingMT(t *testing.T) {
	...
	// assert signed tx do deploy sc
	assert.Equal(t, executor.ROM_ERROR_NO_ERROR, processBatchResponse.Responses[0].Error)
	...
	// assert signed tx to increment counter
	assert.Equal(t, executor.ROM_ERROR_NO_ERROR, processBatchResponse.Responses[1].Error)
