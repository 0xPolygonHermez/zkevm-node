...
func TestBasicState_AddBlock(t *testing.T) {
	...
	_, err = stateDb.Exec(ctx, "DELETE FROM block WHERE block_num = $1", block1.BlockNumber)
	assert.NoError(t, err)
	_, err = stateDb.Exec(ctx, "DELETE FROM block WHERE block_num = $1", block2.BlockNumber)
	assert.NoError(t, err)
}
...
func TestBasicState_AddSequencer(t *testing.T) {
	...
	_, err = stateDb.Exec(ctx, "DELETE FROM sequencer WHERE chain_id = $1", sequencer1.ChainID.Uint64())
	assert.NoError(t, err)
	_, err = stateDb.Exec(ctx, "DELETE FROM sequencer WHERE chain_id = $1", sequencer2.ChainID.Uint64())
	assert.NoError(t, err)
}
