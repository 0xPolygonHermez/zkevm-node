...
type pool_migrationTest0003 struct{}

func (m pool_migrationTest0003) InsertData(db *sql.DB) error {
...
func (m pool_migrationTest0003) RunAssertsAfterMigrationUp(t *testing.T, db *sql.DB) {
...
func (m pool_migrationTest0003) RunAssertsAfterMigrationDown(t *testing.T, db *sql.DB) {
	for _, idx := range indexes {
		// getIndex
		const getIndex = `SELECT count(*) FROM pg_indexes WHERE indexname = $1;`
		row := db.QueryRow(getIndex, idx)
...
func TestMigration0002(t *testing.T) {
	runMigrationTest(t, 2, pool_migrationTest0003{})
}
