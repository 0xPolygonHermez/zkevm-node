...
CREATE INDEX IF NOT EXISTS idx_transaction_from ON pool.transaction (from_address);
...
CREATE INDEX IF NOT EXISTS idx_transaction_wip_status ON pool.transaction (is_wip, status);
...
CREATE INDEX IF NOT EXISTS idx_transaction_from_status ON pool.transaction (from_address, status);
CREATE INDEX IF NOT EXISTS idx_transaction_hash ON pool.transaction (hash);

-- +migrate Down
DROP TABLE pool.whitelisted;

DROP INDEX IF EXISTS pool.idx_transaction_from
