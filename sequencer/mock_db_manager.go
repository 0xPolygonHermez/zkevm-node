...
// GetTransactionsByBatchNumber provides a mock function with given fields: ctx, batchNumber, forkID
func (_m *DbManagerMock) GetTransactionsByBatchNumber(ctx context.Context, batchNumber uint64, forkID uint64) ([]types.Transaction, []uint8, error) {
	ret := _m.Called(ctx, batchNumber, forkID)

	var r0 []types.Transaction
	var r1 []uint8
	var r2 error
	if rf, ok := ret.Get(0).(func(context.Context, uint64, uint64) ([]types.Transaction, []uint8, error)); ok {
		return rf(ctx, batchNumber, forkID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, uint64, uint64) []types.Transaction); ok {
		r0 = rf(ctx, batchNumber, forkID)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(context.Context, uint64, uint64) []uint8); ok {
		r1 = rf(ctx, batchNumber, forkID)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).([]uint8)
		}
