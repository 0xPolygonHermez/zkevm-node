...
func NewTxProfitabilityCheckerBase(
	ethMan etherman,
	state localState, minReward *big.Int,
	intervalAfterWhichBatchSentAnyway time.Duration,
	rewardPercentageToAggregator int64,
) *Base {
	return &Base{
...
func NewTxProfitabilityCheckerAcceptAll(ethman etherman, state localState, intervalAfterWhichBatchSentAnyway time.Duration) *AcceptAll {
	return &AcceptAll{
		EthMan:                            ethman,
		State:                             state,
		IntervalAfterWhichBatchSentAnyway: intervalAfterWhichBatchSentAnyway,
...
func isNewBatchNotAppeared(ctx context.Context, state localState, intervalAfterWhichBatchSentAnyway time.Duration) (bool, error) {
