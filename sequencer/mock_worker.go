...
// NewTxTracker provides a mock function with given fields: tx, counters, ip, forkID
func (_m *WorkerMock) NewTxTracker(tx types.Transaction, counters state.ZKCounters, ip string, forkID uint64) (*TxTracker, error) {
	ret := _m.Called(tx, counters, ip, forkID)

	var r0 *TxTracker
	var r1 error
	if rf, ok := ret.Get(0).(func(types.Transaction, state.ZKCounters, string, uint64) (*TxTracker, error)); ok {
		return rf(tx, counters, ip, forkID)
	}
	if rf, ok := ret.Get(0).(func(types.Transaction, state.ZKCounters, string, uint64) *TxTracker); ok {
		r0 = rf(tx, counters, ip, forkID)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*TxTracker)
		}
	...
	if rf, ok := ret.Get(1).(func(types.Transaction, state.ZKCounters, string, uint64) error); ok {
		r1 = rf(tx, counters, ip, forkID)
	} else {
		r1 = ret.Error(1)
	}
