...
// NewTx provides a mock function with given fields: tx, counters
func (_m *WorkerMock) NewTx(tx types.Transaction, counters state.ZKCounters) (*TxTracker, error) {
	ret := _m.Called(tx, counters)

	var r0 *TxTracker
	if rf, ok := ret.Get(0).(func(types.Transaction, state.ZKCounters) *TxTracker); ok {
		r0 = rf(tx, counters)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*TxTracker)
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(types.Transaction, state.ZKCounters) error); ok {
		r1 = rf(tx, counters)
	} else {
		r1 = ret.Error(1)
	}
