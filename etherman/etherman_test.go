...
func TestForcedBatchEvent(t *testing.T) {
	...
	amount, err := etherman.ZkEVMRollup.GetForcedBatchFee(&bind.CallOpts{Pending: false})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, err = etherman.ZkEVMRollup.ForceBatch(auth, data, amount)
	require.NoError(t, err)
...
func TestSequencedBatchesEvent(t *testing.T) {
	...
	sequences = append(sequences, polygonzkevmrollup.PolygonZkEVMBatchData{
		GlobalExitRoot:     ger,
	...
	_, err = etherman.ZkEVMRollup.SequenceBatches(auth, sequences, auth.From)
	require.NoError(t, err)
...
func TestVerifyBatchEvent(t *testing.T) {
	...
	rawTxs := ""
	tx := polygonzkevmrollup.PolygonZkEVMBatchData{
		GlobalExitRoot:     common.Hash{},
		Timestamp:          initBlock.Time(),
		MinForcedTimestamp: 0,
		Transactions:       common.Hex2Bytes(rawTxs),
	}
	_, err = etherman.ZkEVMRollup.SequenceBatches(auth, []polygonzkevmrollup.PolygonZkEVMBatchData{tx}, auth.From)
	require.NoError(t, err)

	// Mine the tx in a block
	ethBackend.Commit()

	_, err = etherman.ZkEVMRollup.VerifyBatchesTrustedAggregator(auth, uint64(0), uint64(0), uint64(1), [32]byte{}, [32]byte{}, [24][32]byte{})
	require.NoError(t, err)
...
func TestSequenceForceBatchesEvent(t *testing.T) {
	...
	forceBatchData := polygonzkevmrollup.PolygonZkEVMForcedBatchData{
	...
	_, err = etherman.ZkEVMRollup.SequenceForceBatches(auth, []polygonzkevmrollup.PolygonZkEVMForcedBatchData{forceBatchData})
	require.NoError(t, err)
	ethBackend.Commit()
...
func TestSendSequences(t *testing.T) {
	...
	tx, err := etherman.sequenceBatchesRollup(*auth, []ethmanTypes.Sequence{sequence}, auth.From)
	require.NoError(t, err)
