...
func defaultsL1EventProcessors(sync *ClientSynchronizer) *processor_manager.L1EventProcessors {
	...
	poolReorg := common.NewReorgPool(sync.state, sync.etherMan, sync.pool)
	p.Register(etrog.NewProcessorL1SequenceBatches(sync.state, sync, common.DefaultTimeProvider{}, sync.halter, poolReorg))
