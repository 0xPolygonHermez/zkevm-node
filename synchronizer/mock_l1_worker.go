...
// asyncRequestRollupInfoByBlockRange provides a mock function with given fields: ctx, ch, wg, blockRange3, sleepBefore
func (_m *workerMock) asyncRequestRollupInfoByBlockRange(ctx contextWithCancel, ch chan responseRollupInfoByBlockRange, wg *sync.WaitGroup, blockRange3 blockRange, sleepBefore time.Duration) error {
	ret := _m.Called(ctx, ch, wg, blockRange3, sleepBefore)
	...
	if rf, ok := ret.Get(0).(func(contextWithCancel, chan responseRollupInfoByBlockRange, *sync.WaitGroup, blockRange, time.Duration) error); ok {
		r0 = rf(ctx, ch, wg, blockRange3, sleepBefore)
	} else {
		r0 = ret.Error(0)
	}
